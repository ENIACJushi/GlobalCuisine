...
1、在行为包的 `description/states` 定义食用步数 `amp:eat_step`

```
"states": {
  "amp:eat_step" : [ 0, 1, 2, 3, 4, 5 ]
}
```
...
[
  {
    "condition": "query.block_state('amp:eat_step') == 1",
    "components": {
      "minecraft:material_instances": {
        "*": { "texture": "baozi_amp", "render_method": "alpha_test" },
        "eat_1": { "texture": "amp_void", "render_method": "alpha_test" }
      },
      "minecraft:loot": "loot_tables/amp/empty.json"
    }
  },
  {
    "condition": "query.block_state('amp:eat_step') == 2",
    "components": {
      "minecraft:material_instances": {
        "*": { "texture": "baozi_amp", "render_method": "alpha_test" },
        "eat_1": { "texture": "amp_void", "render_method": "alpha_test" },
        "eat_2": { "texture": "amp_void", "render_method": "alpha_test" }
      },
      "minecraft:loot": "loot_tables/amp/empty.json"
    }
  }
...
3、用bb打开模型，UV模式设为逐面UV

![eat_com_3.png](assets/eat_com_3.png)

4、右键方块，点击 `Edit Material Instance`，为其设置材质名。材质名对应第二部隐藏的材质。

![assets/eat_com_4.png](assets/eat_com_4.png)